"""
运行指南：
- 本模块实现推荐任务的 BERT4Rec 序列推荐算法，不直接运行。
...
class BERT4RecAlgorithm:
    """BERT4Rec（基于 Transformer 的序列推荐）。"""
    ...
    def run(
        ...
        logger.info(
            "BERT4Rec initialized hidden_dim=%d num_layers=%d num_heads=%d dropout=%.3f lr=%.6f weight_decay=%.6f epochs=%d patience=%d",
            self.hidden_dim,
            self.num_layers,
            self.num_heads,
            self.dropout,
        ...
        for epoch in range(int(self.epochs)):
            ...
            if count > 0:
                logger.info("BERT4Rec epoch=%d loss=%.6f", epoch + 1, total_loss / count)
            ...
            if val_row_idx:
                ...
                if patience_counter >= self.patience:
                    logger.info("BERT4Rec early stopping at epoch=%d, best_val_ndcg@%d=%.6f", epoch + 1, self.top_k, best_val_ndcg)
                    break
        ...
        if val_row_idx:
            ...
            logger.info("BERT4Rec val_ndcg@%d=%.6f", self.top_k, val_ndcg)
...
class _BERT4RecModel(nn.Module):
    """BERT4Rec 序列推荐模型。"""
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(torch.randn(1, 512, hidden_dim))
    ...
    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        ...
        mask = torch.triu(torch.ones(seq_len, seq_len, device=seq.device) * float("-inf"), diagonal=1)
